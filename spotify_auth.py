...
def save_tokens(access_token, refresh_token):
    ...
    try:
        ...
        os.chmod(TOKEN_FILE, 0o600)  # Restrict permissions (owner read/write only)
...
def get_auth_code(client_id, redirect_uri, scope="user-modify-playback-state user-read-playback-state"):
    """
...
def get_access_token(client_id, client_secret, auth_code=None, redirect_uri=None):
    ...
    if response.status_code == 200:
        ...
        if refresh_token:
            ...
            print(f"✓ Tokens saved for future use")
...
def validate_token_scope(access_token, required_scope):
    ...
    if "user-modify-playback-state" in required_scope or "user-read-playback-state" in required_scope:
        ...
        if response.status_code == 401:
            ...
            try:
                ...
                if "Permissions missing" in error_message or "permissions" in error_message.lower():
...
def authenticate_if_needed(client_id=None, client_secret=None, redirect_uri=None, scope="user-modify-playback-state user-read-playback-state"):
    """
