...
from spotify_auth import (
    CLIENT_ID, CLIENT_SECRET, DEVICE_ID,
    REDIRECT_URI,
...
# Debounce constant: ignore same tag if scanned within this many seconds
DEBOUNCE_SECONDS = 3
...
def main():
    """Main service loop for NFC album player."""
    print("=" * 60)
    ...
    try:
        while True:
            ...
            if (album_uri_raw == last_scanned_uri and
                ...
                elapsed = (current_time - last_scan_time).total_seconds()
                remaining_time = DEBOUNCE_SECONDS - elapsed
                print(f"⚠ Same tag detected. Please wait {remaining_time:.1f} "
                      "seconds before rescanning.")
                time.sleep(1)
                continue
            ...
            # Process the album
            success, message = process_album_scan(access_token, device_id,
                                                  album_uri_raw)
